...
internal class WindowGeometryLedger {
    /** 最近一份已经送达 service、可用于 resize 去重的窗口尺寸。 */
    private val appSizes = mutableMapOf<String, Pair<Int, Int>>()
    /** 最近一份已经在屏幕上落定、可由同步窗口 API 读取的窗口事实。 */
    private val appSystemInfo = mutableMapOf<String, JSONObject>()

    /** 返回 service 已知的最近窗口轴；尚无基线时返回 null。 */
    @Synchronized
    fun lastKnownIsLandscape(appId: String): Boolean? =
        appSizes[appId]?.let { (width, height) -> width > height }

    @Synchronized
    fun lastKnownSystemInfo(appId: String): JSONObject? =
        appSystemInfo[appId]?.let { JSONObject(it.toString()) }

    /**
     * 窗口完成布局时立即更新同步 API 使用的事实，不推进 service 的 resize 去重基线。
     * 页面资源可能晚于窗口布局就绪；这时 onShow 仍应读到新尺寸，而同一份几何必须保留为待发送状态。
     */
    @Synchronized
    fun observeSettled(appId: String, systemInfo: JSONObject) {
        appSystemInfo[appId] = JSONObject(systemInfo.toString())
    }

    /**
     * 只判，不记。
     * 基线推进的唯一入口是 [record]：资源未就绪时消息虽已排队，却还没有送达 service；提前记账会让排队期间的同尺寸布局被误判成重复。
     */
    @Synchronized
    fun decide(
        appId: String,
