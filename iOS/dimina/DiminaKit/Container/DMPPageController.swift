...
/// One window transition may expose an unusable interpolated size before its final window bounds.
/// The current generation can claim exactly one reportable geometry; stale completions and the
/// completion following an already-reported target become inert.
struct DMPTransitionGeometryLedger {
    ...
    mutating func claim(generation candidate: Int, size: CGSize) -> Bool {
        guard candidate == generation,
              settledGeneration != candidate,
              Self.isReportable(size)
        else { return false }
        settledGeneration = candidate
        return true
    }

    mutating func finishWithoutGeometry(generation candidate: Int) -> Bool {
        guard candidate == generation, settledGeneration != candidate else { return false }
        settledGeneration = candidate
        return true
    }

    static func isReportable(_ size: CGSize) -> Bool {
        size.width.isFinite && size.height.isFinite && size.width > 0 && size.height > 0
    }

    static func ownsGeometryRequest(
        transitionOwner: Int?,
        activeOwner: Int?
    ) -> Bool {
        guard let transitionOwner else { return false }
        return transitionOwner == activeOwner
    }
}
...
public class DMPPageController: UIViewController {
    ...
    private var transitionGeometryLedger = DMPTransitionGeometryLedger()
    ...
    /// 开始），新 tab 的 view 才被挂进已经在窗口里的容器，它的 `viewDidAppear` 因此可能早于 `notifyPageShowAfterOrientationSettles` 登记 pendingPageShow。
    ...
    static func isReportableWindowSize(_ size: CGSize) -> Bool {
        DMPTransitionGeometryLedger.isReportable(size)
    }
    ...
    public override func viewWillTransition(
        ...
        guard navigator?.pageOrientationSupport == .supported else { return }
        let transitionGeneration = transitionGeometryLedger.begin()
        let visibilityGeneration = pageShowGeneration
        let geometryRequestOwner = activeGeometryRequest?.generation
        ...
        coordinator.animate(alongsideTransition: { [weak self] _ in
            ...
            guard self.transitionGeometryLedger.claim(
                generation: transitionGeneration,
                size: size
            ) else {
                DMPLogger.debug(
                    "viewWillTransition got a deferred or stale size \(size) for \(self.pagePath)"
                )
                return
            }
            self.dispatchTransitionGeometry(
                size,
                visibilityGeneration: visibilityGeneration,
                callSite: "viewWillTransition"
            )
        }, completion: { [weak self] _ in
            guard let self else { return }
            self.rememberDisplayedOrientationIfNeeded()

            // An impossible interpolated size is not a terminal event. Completion owns the actual
            // window bounds and closes the same transition transaction exactly once.
            let finalSize = self.viewIfLoaded?.window?.bounds.size ?? .zero
            if self.transitionGeometryLedger.claim(
                generation: transitionGeneration,
                size: finalSize
            ) {
                self.dispatchTransitionGeometry(
                    finalSize,
                    visibilityGeneration: visibilityGeneration,
                    callSite: "viewWillTransitionCompletion"
                )
            }
            else if self.transitionGeometryLedger.finishWithoutGeometry(
                generation: transitionGeneration
            ) {
                // The transition ended without any window fact. Retire the request so a later
                // appearance/device event can submit a fresh one instead of joining a dead owner.
                // A newer request may have appeared while this transition was running; the old
                // completion cannot retire that owner.
                if DMPTransitionGeometryLedger.ownsGeometryRequest(
                    transitionOwner: geometryRequestOwner,
                    activeOwner: self.activeGeometryRequest?.generation
                ) {
    ...
    private func dispatchTransitionGeometry(
        _ size: CGSize,
        visibilityGeneration: Int,
        callSite: String
    ) {
        Task { @MainActor [weak self] in
            guard let self, self.pageShowGeneration == visibilityGeneration else { return }
            self.settleGeometryRequest(targetSize: size)
            // pageShow must precede the report so service can bind Page.onResize to this owner.
            await self.flushPendingPageShow(targetSize: size)
            await self.notifyPageResize(targetSize: size, callSite: callSite)
        }
    }
