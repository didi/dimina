...
final class DMPNavigatorCapsuleTests: XCTestCase {
    ...
    func testTransitionGeometryFallsBackToTheFinalWindowSizeExactlyOnce() {
        var ledger = DMPTransitionGeometryLedger()
        let generation = ledger.begin()

        XCTAssertFalse(ledger.claim(generation: generation, size: CGSize(width: -66, height: 1311)))
        XCTAssertTrue(ledger.claim(generation: generation, size: CGSize(width: 393, height: 852)))
        XCTAssertFalse(ledger.claim(generation: generation, size: CGSize(width: 393, height: 852)))
    }

    func testTransitionGeometryRejectsACompletionFromAnOlderTransition() {
        var ledger = DMPTransitionGeometryLedger()
        let stale = ledger.begin()
        let current = ledger.begin()

        XCTAssertFalse(ledger.claim(generation: stale, size: CGSize(width: 393, height: 852)))
        XCTAssertTrue(ledger.claim(generation: current, size: CGSize(width: 852, height: 393)))
    }

    func testTransitionGeometryCanRetireARequestThatNeverProducedValidBounds() {
        var ledger = DMPTransitionGeometryLedger()
        let generation = ledger.begin()

        XCTAssertFalse(ledger.claim(generation: generation, size: .zero))
        XCTAssertTrue(ledger.finishWithoutGeometry(generation: generation))
        XCTAssertFalse(ledger.claim(generation: generation, size: CGSize(width: 393, height: 852)))
    }

    func testTransitionCompletionCannotRetireANewerGeometryRequest() {
        XCTAssertTrue(DMPTransitionGeometryLedger.ownsGeometryRequest(
            transitionOwner: 7,
            activeOwner: 7
        ))
        XCTAssertFalse(DMPTransitionGeometryLedger.ownsGeometryRequest(
            transitionOwner: 7,
            activeOwner: 8
        ))
        XCTAssertFalse(DMPTransitionGeometryLedger.ownsGeometryRequest(
    ...
    func testNativeCrossMiniProgramEntriesUseTheRoutePageDispatcher() throws {
        ...
        XCTAssertEqual(iosNavigator.components(separatedBy: "pageLifecycle?.onHide(webviewId:").count - 1, 1)
        XCTAssertEqual(iosNavigator.components(separatedBy: "pageLifecycle?.onShow(webviewId:").count - 1, 1)
