...
本文以微信小程序当前的 [WXML 事件](https://developers.weixin.qq.com/miniprogram/dev/framework/view/wxml/event.html) 与 [`label`](https://developers.weixin.qq.com/miniprogram/dev/component/label.html) 文档为公开契约基准。文档未定义的部分（多指进入后的取消时机、label 对隐藏与禁用控件的处理、容器事件的归属）以微信开发者工具上的可复现行为为准，并在第 7 节集中列出。
...
| `tap` | 单指按下并抬起，期间未超过位移阈值、未被长按或多指抑制 | 由触摸序列合成，不来自原生 `click` |
| `longtap` | 单指按住超过阈值 | 不抑制随后的 `tap`，属弃用兼容行为 |
| `longpress` | 单指按住超过阈值 | 节点绑定了 `longpress` 时抑制本次 `tap` |
| `canceltap` | 位移超过阈值，或第二根手指加入，或序列被取消 | 越过位移阈值后每次 `touchmove` 都补一条；`touchcancel` 路径最多一条 |
...
- 位移超过阈值后本次序列不再产生 `tap`；此后每一次 `touchmove` 都伴随一条 `canceltap`。
- `longpress` 触发后抑制本次 `tap`；抑制标记记录在原生事件上，祖先节点观察到同一次序列时也不再合成自己的 `tap`。`longtap` 不抑制。
- `tap`、`longpress` 与 `longtap` 是单指手势。第二根手指加入时抑制本次 `tap`、派发 `canceltap` 并取消长按计时器，本次序列不再触发 `longpress` 与 `longtap`，也不重置起点。
...
节点卸载时若原生序列仍在进行，所有者延迟到真实的 `touchend` / `touchcancel` 再摘除，不伪造终态。
...
- 多指进入后立即抑制 `tap` 并派发 `canceltap`，微信文档未定义该时机。
