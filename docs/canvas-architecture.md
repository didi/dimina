...
Dimina 同时支持旧版 `wx.createCanvasContext()`、`<canvas type="2d">` / `SelectorQuery.fields({ node: true })`，以及小游戏和离屏 Canvas。三条入口最终共用渲染层的真实 `HTMLCanvasElement`，但定位方式、调用协议和生命周期不同。
...
渲染层编码后，Android、iOS 和 HarmonyOS 在各自的后台执行域解码并发布临时文件。每个请求都绑定 app owner 和 runtime generation：
...
| 范围 | 文件 |
| --- | --- |
| 编译到 `dd-canvas` | `fe/packages/compiler/src/core/view-compiler.js` |
