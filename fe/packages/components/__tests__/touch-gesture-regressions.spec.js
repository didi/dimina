...
describe('touch gesture state regressions', () => {
	...
	it('cancels single-touch tap and longpress when a second touch joins the sequence', async () => {
		vi.useFakeTimers()
		const info = makeInfo({
	...
	it('does not restart a single-touch gesture while contacts from a cancelled multi-touch sequence remain', async () => {
		vi.useFakeTimers()
