...
describe('tap is synthesized from the touch sequence', () => {
	it('synthesizes tap on touchend: detail.x/y come from touchend, touches/changedTouches come from touchstart', async () => {
		...
		const lifted = touchPoint({ identifier: 7, pageX: 99, pageY: 88, clientX: 91, clientY: 81, screenX: 92, screenY: 82, force: 0.6 })
		...
		expect(tapCall.event.detail.x).toBe(99)
		expect(tapCall.event.detail.y).toBe(88)
...
describe('canceltap', () => {
	it('dispatches canceltap after every post-threshold touchmove', async () => {
