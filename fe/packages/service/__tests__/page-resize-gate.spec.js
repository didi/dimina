 ...
 *
 * 同一 16ms 窗口内到达的多次上报合并成一次结算：窗口通道保留最后一份有触发资格的报告，页面通道按 bridgeId 保留各自最后一份有触发资格的报告。
...
describe('page resize dispatch gate', () => {
	...
	beforeEach(() => {
		...
		runtime.resizeGate.pendingWindowEvent = null
	...
	it('merges pageResize calls within 16ms while retaining each page owner\'s own geometry', async () => {
		const listener = await registerWindowListener()
		...
		// 页面通道按 owner 合并；B 的报告不能替换 A 自己取得派发资格时的几何。
		expect(receivedA).toHaveLength(1)
		expect(receivedA[0].size).toEqual(PORTRAIT)
		expect(receivedA[0].deviceOrientation).toBe('portrait')

		expect(receivedB).toHaveLength(1)
		expect(receivedB[0].size).toEqual(LANDSCAPE)
	})

	it('keeps window and page payloads bound to the reports that qualified for each channel', async () => {
		const listener = await registerWindowListener()
		const receivedAuto = mountPage('bridge-auto')
		const receivedFixed = mountPage('bridge-fixed')

		runtime.pageResize({
			bridgeId: 'bridge-auto',
			size: PORTRAIT,
			deviceOrientation: 'portrait',
			pageOrientation: { originalPageOrientation: 'auto' },
		})
		runtime.pageResize({
			bridgeId: 'bridge-fixed',
			size: LANDSCAPE,
			deviceOrientation: 'landscape',
			pageOrientation: { originalPageOrientation: 'portrait' },
		})
		...
		expect(listener).toHaveBeenCalledTimes(1)
		expect(listener).toHaveBeenCalledWith({ size: PORTRAIT, deviceOrientation: 'portrait' })
		expect(receivedAuto).toEqual([{ size: PORTRAIT, deviceOrientation: 'portrait' }])
		expect(receivedFixed).toHaveLength(0)
	})
